...
extension Book {
    ...
    #if DEBUG
    static let lastWeek: Date = Calendar.current.date(byAdding: .day, value: -7, to: Date.now) ?? Date.now
    static let lastMonth: Date = Calendar.current.date(byAdding: .month, value: -1, to: Date.now) ?? Date.now
